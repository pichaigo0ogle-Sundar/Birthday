...
# ═══════════════════════════════════════════════════════════════
# STAGE 4 — CINEMATIC BIKE JOURNEY
# ═══════════════════════════════════════════════════════════════
def stage_4() -> None:
    html = f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8">
...
.road-glow{{position:absolute;top:0;left:0;right:0;height:1px;
  background:linear-gradient(90deg,transparent,rgba(139,92,246,.55),rgba(59,130,246,.55),transparent);
  box-shadow:0 0 22px rgba(139,92,246,.45);}}
.dashes{{position:absolute;top:50%;left:0;right:0;height:4px;transform:translateY(-50%);
  background:repeating-linear-gradient(90deg,transparent 0,transparent 34px,rgba(251,191,36,.62) 34px,rgba(251,191,36,.62) 68px);
...
.wheel::before{{content:'';position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
  border-radius:50%;border:2px solid;opacity:.38;width:55%;height:55%;}}
.wheel::after{{content:'';position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
  border-radius:50%;width:7px;height:7px;}}
.wr{{left:0;width:46px;height:46px;border-color:#8b5cf6;color:#8b5cf6;box-shadow:0 0 20px rgba(139,92,246,.85);}}
.wr::after{{background:#fbbf24;box-shadow:0 0 8px #fbbf24;}}
.wf{{right:0;width:46px;height:46px;border-color:#6366f1;color:#6366f1;box-shadow:0 0 20px rgba(99,102,241,.85);}}
.wf::after{{background:#fbbf24;box-shadow:0 0 8px #fbbf24;}}
@keyframes spin{{from{{transform:rotate(0deg);}}to{{transform:rotate(360deg);}}}}
.frame{{position:absolute;bottom:24px;left:20px;width:94px;height:46px;}}
.frame::before{{content:'';position:absolute;inset:0;
  border-left:4px solid #7c3aed;border-bottom:4px solid #7c3aed;border-top:3px solid #8b5cf6;
  transform:skewX(-13deg);box-shadow:0 0 12px rgba(124,58,237,.65);}}
.hbar{{position:absolute;bottom:42px;right:14px;width:3px;height:30px;
  background:#a78bfa;transform:rotate(12deg);box-shadow:0 0 10px rgba(167,139,250,.9);border-radius:2px;}}
.hbar::after{{content:'';position:absolute;top:0;left:-13px;width:28px;height:3px;
  background:#a78bfa;border-radius:2px;box-shadow:0 0 8px rgba(167,139,250,.9);}}
.seat{{position:absolute;bottom:46px;left:30px;width:34px;height:6px;
  background:#c084fc;border-radius:6px;box-shadow:0 0 10px rgba(192,132,252,.9);}}
.rider{{position:absolute;bottom:46px;left:40px;width:22px;height:38px;
  background:linear-gradient(#fbbf24 0%,#fbbf24 36%,#6366f1 36%,#4338ca 100%);border-radius:10px 10px 4px 4px;}}
.rhead{{position:absolute;top:-17px;left:50%;transform:translateX(-50%);
  width:19px;height:19px;background:#fde68a;border-radius:50%;box-shadow:0 0 14px rgba(253,230,138,.6);}}
.helmet{{position:absolute;top:-9px;left:50%;transform:translateX(-50%);
  width:22px;height:13px;background:#7c3aed;border-radius:10px 10px 0 0;}}
.hlight{{position:absolute;bottom:24px;right:-3px;width:0;height:0;
  border-left:70px solid rgba(251,191,36,.22);
  border-top:14px solid transparent;border-bottom:14px solid transparent;
  filter:blur(5px);animation:hp .45s ease-in-out infinite;}}
.uglow{{position:absolute;bottom:-10px;left:0;right:0;height:12px;
  background:radial-gradient(ellipse,rgba(139,92,246,.5) 0%,transparent 70%);
  filter:blur(7px);animation:hp .45s ease-in-out infinite;}}
@keyframes hp{{0%,100%{{opacity:.65;}}50%{{opacity:1;}}}}
.ntrl{{position:absolute;right:100%;top:50%;width:85px;height:3px;transform:translateY(-50%);
  background:linear-gradient(90deg,transparent,rgba(139,92,246,.75));filter:blur(2px);border-radius:2px;}}
.exhaust{{position:absolute;bottom:24px;left:-10px;}}
.puff{{position:absolute;bottom:0;left:0;width:11px;height:11px;
  background:rgba(139,92,246,.35);border-radius:50%;animation:puff .85s ease-out infinite;}}
...
<div class="road"><div class="road-glow"></div><div class="dashes"></div></div>
<div class="bike-wrap" style="--dur:5.5s;--del:0s;">
  <div class="bike">
    <div class="wr wheel"></div><div class="wf wheel"></div>
    <div class="frame"></div><div class="hbar"></div><div class="seat"></div>
    <div class="rider"><div class="rhead"></div><div class="helmet"></div></div>
    <div class="hlight"></div><div class="uglow"></div><div class="ntrl"></div>
    <div class="exhaust"><div class="puff"></div><div class="puff"></div><div class="puff"></div></div>
  </div>
</div>
<div class="bike-wrap" style="--dur:5.5s;--del:2.75s;bottom:77px;">
  <div class="bike" style="transform:scale(.78);opacity:.62;">
    <div class="wr wheel"></div><div class="wf wheel"></div>
    <div class="frame"></div><div class="hbar"></div><div class="seat"></div>
    <div class="rider"><div class="rhead"></div><div class="helmet"></div></div>
    <div class="hlight"></div><div class="uglow"></div><div class="ntrl"></div>
    <div class="exhaust"><div class="puff"></div><div class="puff"></div></div>
  </div>
</div>
<div class="stage-root content">
  <p class="eyebrow">✦ &nbsp; The Journey &nbsp; ✦</p>
  <p class="quote">"Some journeys become unforgettable<br>because of the person beside us."</p>
</div>
<script>
{PARTICLE_JS}
(function(){{
  ...
  for(var i=0;i<130;i++){{
    var s=document.createElement('div');s.className='star';
    var sz=Math.random()*2.5+.3;
    s.style.cssText='width:'+sz+'px;height:'+sz+'px;top:'+Math.random()*62+'%;left:'+Math.random()*100+'%;animation-duration:'+(Math.random()*3+2)+'s;animation-delay:'+(Math.random()*3)+'s;';
    c.appendChild(s);
  }}
}})();
(function(){{
  var city=document.getElementById('city');
  [[0,75,38],[45,105,50],[100,65,34],[155,115,55],[220,55,28],[280,95,46],[340,110,54],[405,72,36],[465,98,49],[530,60,30],[595,120,60],[660,82,41],[728,95,47],[795,68,34]].forEach(function(b){{
    var el=document.createElement('div');
    el.style.cssText='position:absolute;bottom:0;left:'+b[0]+'px;height:'+b[1]+'px;width:'+b[2]+'px;background:#c4b5fd;border-radius:2px 2px 0 0;';
    city.appendChild(el);
    for(var w=0;w<Math.floor(b[1]/20);w++){{
      var win=document.createElement('div'),lit=Math.random()>.5;
      win.style.cssText='position:absolute;width:6px;height:6px;background:'+(lit?'rgba(251,191,36,.85)':'rgba(255,255,255,.1)')+';border-radius:1px;bottom:'+(w*18+8)+'px;left:50%;transform:translateX(-50%);';
      if(lit)win.style.boxShadow='0 0 6px rgba(251,191,36,.6)';
      el.appendChild(win);
    }}
  }});
}})();
(function(){{
  setInterval(function(){{
    ...
    var h=Math.random()*55+100, w=Math.random()*130+60;
    streak.style.cssText='position:fixed;bottom:'+h+'px;left:-150px;width:'+w+'px;height:2px;background:linear-gradient(90deg,transparent,rgba(251,191,36,.78),transparent);z-index:4;border-radius:2px;box-shadow:0 0 6px rgba(251,191,36,.4);';
    document.body.appendChild(streak);
    var start=null;
    (function move(ts){{if(!start)start=ts;var p=(ts-start)/1100;streak.style.left=(p*120-10)+'%';streak.style.opacity=''+(p<.1?p*10:p>.9?(1-p)*10:1);if(p<1)requestAnimationFrame(move);else streak.remove();}})();
  }},750);
}})();
</script>
</body></html>"""
...
# ═══════════════════════════════════════════════════════════════
# STAGE 6 — AUTO-CYCLING QUOTE CAROUSEL
# ═══════════════════════════════════════════════════════════════
def stage_6() -> None:
    # ⭐ REPLACE: Edit quotes below to personalise
    quotes = [
        {"text": "True beauty is not only seen.<br>It is felt in kindness, warmth, and presence.",
         "accent": "#e879f9"},
        {"text": "Some souls naturally bring peace, light, and happiness wherever they go.",
         "accent": "#60a5fa"},
        {"text": "Beautiful people are remembered.<br>But beautiful hearts are unforgettable.",
         "accent": "#fbbf24"},
        {"text": "You have a rare gift — the ability to make every moment feel meaningful.",
         "accent": "#a78bfa"},
    ]
...
<meta name="viewport" content="width=device-width,initial-scale=1">
<style>
@import url('https://fonts.googleapis.com/css2?family=Cinzel+Decorative:wght@700;900&family=Cinzel:wght@400;700&family=Playfair+Display:ital,wght@1,400;1,700&display=swap');
{ENTRY_CSS}
*{{margin:0;padding:0;box-sizing:border-box;}}
body{{background:#030312;min-height:100vh;overflow:hidden;
  display:flex;flex-direction:column;align-items:center;justify-content:center;position:relative;}}
...
@keyframes qOut{{from{{opacity:1;transform:translateY(0) scale(1);filter:blur(0);}}to{{opacity:0;transform:translateY(-45px) scale(.93);filter:blur(7px);}}}}
.q-card::before{{content:'';position:absolute;left:0;top:15%;height:70%;width:4px;
  background:linear-gradient(to bottom,var(--accent),transparent);border-radius:0 3px 3px 0;transition:height .4s,top .4s;}}
...
<div class="stage-root wrap">
  <p class="eyebrow">✦ &nbsp; Words For You &nbsp; ✦</p>
  <h2 class="sec-title">Beautiful Truths</h2>
  <div id="qSlot"></div>
...
var cur=0, timer=null, DUR=4500;
function h2r(hex,a){{
  var r=parseInt(hex.slice(1,3),16),g=parseInt(hex.slice(3,5),16),b=parseInt(hex.slice(5,7),16);
  return 'rgba('+r+','+g+','+b+','+a+')';
}}
function buildDots(){{
  var el=document.getElementById('qDots');
  quotes.forEach(function(q,i){{
    var d=document.createElement('div');d.className='q-dot'+(i===0?' live':'');
    d.style.setProperty('--a',q.accent);d.onclick=function(){{showQ(i);}};el.appendChild(d);
  }});
}}
function showQ(idx){{
  var slot=document.getElementById('qSlot');
  var old=slot.querySelector('.q-card');
  if(old){{old.classList.add('exit');setTimeout(function(){{old.remove();}},550);}}
  cur=idx;
  document.getElementById('spot').style.background=
    'radial-gradient(circle,'+h2r(quotes[idx].accent,.09)+' 0%,transparent 70%)';
  document.querySelectorAll('.q-dot').forEach(function(d,i){{d.classList.toggle('live',i===idx);}});
  setTimeout(function(){{
    var card=document.createElement('div');card.className='q-card';
    card.style.setProperty('--accent',quotes[idx].accent);
    card.style.setProperty('--glow',h2r(quotes[idx].accent,.18));
    card.innerHTML='<div class="qmark">&ldquo;</div><p class="qtext">'+quotes[idx].text+'</p>';
...
function resetTimer(){{
  ...
  f.style.transition='width '+DUR+'ms linear';f.style.width='0%';
  clearTimeout(timer);timer=setTimeout(function(){{showQ((cur+1)%quotes.length);}},DUR);
}}
buildDots(); showQ(0);
</script>
</body></html>"""
